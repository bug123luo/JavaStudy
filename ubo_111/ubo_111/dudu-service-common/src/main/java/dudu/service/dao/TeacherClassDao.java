...
import dudu.service.dao.bean.TeacherClassBean;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.List;

public class TeacherClassDao extends DuduDaoSupport {

    private static Logger logger = LoggerFactory.getLogger(TeacherClassDao.class);

    public TeacherClassDao() {

    }

    public void saveOrUpdate(TeacherClassBean teacherClassBean) throws DaoException {
        super.saveOrUpdate(teacherClassBean);
    }

    public List<TeacherClassBean> getByTeaId(Long teacherId)
            throws DaoException {

        String hql = String.format("FROM TeacherClassBean tea WHERE " +
                        "tea.teacherId=\'%s\'",
                teacherId);

        logger.debug(".getByTeacherId hql = {}", hql);
        return super.query(hql);
    }

    public List<TeacherClassBean> getByClassId(Integer classId)
            throws DaoException {
